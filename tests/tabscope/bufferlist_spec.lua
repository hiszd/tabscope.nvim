...
it("adds buffer to tab's list", function()
  ...
  describe("cwd", function()
    it("is a function", function()
      assert.is_function(bufferlist.cwd)
    end)
    ...
    it("can be called with scope option", function()
      assert.has_no_error(function()
        bufferlist.cwd({ scope = "tab" })
      end)
      assert.has_no_error(function()
        bufferlist.cwd({ scope = "global" })
      end)
    end)
  end)
  ...
  describe("open", function()
    it("is a function", function()
      assert.is_function(bufferlist.open)
    end)
    ...
    it("can be called with scope option", function()
      assert.has_no_error(function()
        bufferlist.open({ scope = "tab" })
      end)
      assert.has_no_error(function()
        bufferlist.open({ scope = "global" })
      end)
    end)
  end)
  ...
  describe("picker module", function()
    ...
    before_each(function()
      p = require("tabscope.bufferlist.picker")
    end)

    describe("list_directories", function()
      it("can be called with max depth of 1", function()
        local root = "/home/zion/.config"
        local dirs = p.list_directories(root, 1)
    ...
    describe("list_files", function()
      it("can be called with max depth of 1", function()
        local root = "/home/zion/.config"
        local files = p.list_files(root, 1)
        assert.is_table(files)
      end)
    end)

    describe("get_picker_backend", function()
      it("returns a valid backend", function()
        local backend = p.get_picker_backend()
        assert.is_true(backend == "snacks" or backend == "telescope" or backend == "vim_ui")
      end)
    end)
  end)
end)